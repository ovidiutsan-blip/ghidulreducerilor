#!/usr/bin/env python3
"""Test: captureaza imaginea unui singur deal mathaus via Playwright response interception"""
...
data = json.loads((REPO / 'data' / 'deals.json').read_text(encoding='utf-8'))
...
with sync_playwright() as p:
    ...
    try:
        ...
        print(f'Captured {len(captured)} CDN images total')
        if not captured:
            print('DOM imgs (naturalWidth > 0):')
            imgs = pg.evaluate("""
                [...document.querySelectorAll('img')]
                .map(i=>({src: i.src.substring(0,80), w: i.naturalWidth, h: i.naturalHeight}))
                .filter(i=>i.src && i.w > 0)
            """)
            for im in imgs[:8]:
                print(' ', im)
    except Exception as e:
...
print()
print('Result:', list(captured.keys())[:3])
